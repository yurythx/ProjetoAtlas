...
export function ArticlesListClient({ initialArticles = [], initialHasNext = false }: ArticlesListClientProps) {
  ...
  const {
    ...
    initialPageParam: 1,
    // Use server-fetched data as initial cache when not searching
    initialData: !isSearching && initialArticles.length > 0
      ? {
          pages: [{ results: initialArticles, next: initialHasNext ? "2" : null }],
          pageParams: [1],
        }
      : undefined,
    getNextPageParam: (lastPage) => {
      const next = (lastPage as { next?: string | null })?.next
    ...
    <div className="py-8 sm:py-12 space-y-8 sm:space-y-12">
      ...
      {error && (
        <div className="text-center py-12 border-2 border-destructive/20 rounded-3xl bg-destructive/5" role="alert" aria-live="assertive">
          <p className="text-destructive font-medium">Erro ao carregar artigos. Tente novamente mais tarde.</p>
        </div>
      )}
        ...
        <div className="text-center py-16 sm:py-20 border-2 border-dashed rounded-3xl bg-muted/10">
          ...
          <p className="text-muted-foreground max-w-md mx-auto">
            {debouncedSearchTerm.trim()
              ? `Não encontramos artigos com "${debouncedSearchTerm.trim()}". Tente ajustar sua busca.`
